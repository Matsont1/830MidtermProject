...
if tab == "Lift Prediction Calculator":
    ...
    """)
        ...
        # Assume df_new_powerlift is already loaded
    ...
    if "last_input_change" not in st.session_state:
        st.session_state.last_input_change = time.time()
    ...
    with st.form(key="lift_prediction_form"):
        st.session_state.inputs["input_lift"] = st.selectbox(
            "Which lift do you want to predict?", ["Squat", "Bench", "Deadlift"], index=["Squat", "Bench", "Deadlift"].index(st.session_state.inputs["input_lift"])
        )
        st.session_state.inputs["bodyweight"] = st.number_input(
            "Enter your body weight (kg):", min_value=20.0, max_value=200.0, step=0.1, value=st.session_state.inputs["bodyweight"]
        )
        st.session_state.inputs["sex"] = st.selectbox(
            "Select your sex:", ["Male", "Female"], index=["Male", "Female"].index(st.session_state.inputs["sex"])
        )
        if st.session_state.inputs["input_lift"] == "Squat":
            st.session_state.inputs["bench"] = st.number_input(
                "Enter your Bench (kg):", min_value=20.0, max_value=600.0, step=0.1, value=st.session_state.inputs["bench"]
            )
            st.session_state.inputs["deadlift"] = st.number_input(
                "Enter your Deadlift (kg):", min_value=20.0, max_value=600.0, step=0.1, value=st.session_state.inputs["deadlift"]
            )
        elif st.session_state.inputs["input_lift"] == "Bench":
            st.session_state.inputs["squat"] = st.number_input(
                "Enter your Squat (kg):", min_value=20.0, max_value=600.0, step=0.1, value=st.session_state.inputs["squat"]
            )
            st.session_state.inputs["deadlift"] = st.number_input(
                "Enter your Deadlift (kg):", min_value=20.0, max_value=600.0, step=0.1, value=st.session_state.inputs["deadlift"]
            )
    ...
    # Handle debounced predictions
    current_time = time.time()
    if current_time - st.session_state.last_input_change > DEBOUNCE_INTERVAL:
        if submit:
